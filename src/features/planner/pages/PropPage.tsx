...
import { updateProp, deleteProp } from '../services/propService'
...
export function PropPage() {
  ...
  const { prop, loading, refresh } = useProp(id)
  ...
  async function handleShopSave() {
    ...
    setShopSaving(true)
    try {
      await replaceSourceShopItems('prop', id, user.uid, editShopItems.map(({ item, forWhat, link, price, quantity }) => ({ item, forWhat, link, price, quantity })))
      const items = await getSourceShopItems('prop', id)
      setShopItems(items)
  ...
  useEffect(() => {
    if (prop) {
      ...
      setIsConsumable(prop.isConsumable || false)
      setReferences(prop.references || [])
      setWorkPlan(prop.workPlan || [])
      setAssignedTo(prop.assignedTo || '')
    }
  }, [prop])

  useEffect(() => {
    if (!id) return
    getSourceShopItems('prop', id).then(setShopItems).catch(() => {})
  }, [id])

  if (!user) return <PageShell requiredAuth isAuthenticated={false} />
  if (loading) return <PageShell loading />
  if (!prop) return <PageShell><div className={styles.page}>Реквизит не найден</div></PageShell>

  const canEdit = prop.userId === user.uid || prop.assignedTo === user.uid

  async function startEditing() {
    try {
      const items = await getSourceShopItems('prop', id!)
      setShoppingList(items.map(({ item, forWhat, link, price, quantity }) => ({ id: crypto.randomUUID(), item, forWhat, link, price: price || 0, quantity, userId: '', sourceType: 'prop', sourceId: '', linkedTo: [], createdAt: Date.now() })))
      setEditing(true)
    } catch {}
  }
  ...
  async function handleSave() {
    ...
    try {
      ...
      await replaceSourceShopItems('prop', id, user.uid, shoppingList.map(({ item, forWhat, link, price, quantity }) => ({ item, forWhat, link, price, quantity })))
      await refresh()
      setEditing(false)
    } catch (err) {
    ...
    return (
      <PageShell>
        <div className={styles.page}>
          <h1 className={styles.title}>Редактирование реквизита</h1>
      ...
      <div className={styles.page}>
        ...
        <div className={styles.viewHeader}>
          <div>
            ...
            <p className={styles.character}>×{prop.quantity} {prop.isConsumable ? '· Расходный материал' : ''}</p>
            {prop.description && <p className={styles.date}>{prop.description}</p>}
            {prop.assignedTo && prop.assignedTo !== prop.userId && (
              <p className={styles.date}>Назначен: {prop.assignedTo}</p>
            )}
          </div>
          {canEdit && (
            ...
            <>
              {canEdit && (
                <button onClick={async () => {
                  try {
                    const items = await getSourceShopItems('prop', id!)
                    setEditShopItems(items.map(({ item, forWhat, link, price, quantity }) => ({ id: crypto.randomUUID(), item, forWhat, link, price: price || 0, quantity, userId: '', sourceType: 'prop', sourceId: '', linkedTo: [], createdAt: Date.now() })))
        ...
        {(prop.workPlan || []).length > 0 && (
          <section className={styles.section}>
            <h2 className={styles.sectionTitle}>План работ</h2>
            <RenderTree nodes={prop.workPlan || []} />
          </section>
        )}
      </div>
    </PageShell>
